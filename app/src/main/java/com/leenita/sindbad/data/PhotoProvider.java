...
public class PhotoProvider
{
	...
	{
		try {
			File cacheDir = SindbadApp.getAppContext().getExternalCacheDir();
			if (cacheDir == null){//cache dir not available
				cacheDir = SindbadApp.getAppContext().getCacheDir();
			}
			...
			ImageLoaderConfiguration.Builder builder = new Builder(SindbadApp.getAppContext());
...
//			builder.memoryCache(new UsingFreqLimitedMemoryCache(2000000));
			...
			try {L.disableLogging();} catch (Exception ignored) {}
	...
	private void checkLastCacheClearTimestamp()
	{
		try {
			...
			long timenow = SindbadApp.getTimestampNow();
			// compare times
